...
import { isBrowserIE, convertIntoArray } from './ie-helpers';
...
export default class ProductDetails {
    ...
    productOptionsChanged2(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            setTimeout(function(){ 
                var change_img = $('.productView-images .productView-img-container img').attr('src');
            ...
            $.each($('[data-product-option-change-2] [data-product-attribute] select option'), function(i) {
                var el;
                if ($(this).is(":selected") == true) {
                    el = $(this).attr('value');
                }
                $.each($('.productView-options [data-product-option-change] [data-product-attribute] select option'), function(i) {
                    var op = $(this).attr('value');
                    if(el == op){
